    ...
    >
      ...
      >
        <div className="flex justify-between items-center">
          {name}
          <span
            className={`bg-sand ml-20 lg:ml-0 rounded-full flex-shrink-0 w-[3.12rem] h-[3.12rem] md:w-[4rem] md:h-[4rem] flex items-center justify-center text-26 md:text-30 font-semibold transition-transform duration-400 transform-gpu will-change-transform ${isOpen ? "rotate-45" : "rotate-0"}`}
          >
            +
          </span>
        </div>
        ...
        {isOpen && (
          ...
          >
            <div className="border-t border-lightGreen p-15 md:p-20">
              <p className="text-darkGreen font-normal text-12 md:text-16 lg:text-18">{descr}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
